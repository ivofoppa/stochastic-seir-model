...
pSE <- function(t,hhs,HHSIarr_list,HHStypePopulation,R0min,R0max,corr,infectiousPeriodMean,HHSfact){ ## k is age group, I is matrix of # infectious by latent/infectious time type
  ...
  beta <- beta(t,R0min,R0max,corr,infectiousPeriodMean)
...
newInfect <- function(t,hhs,HHSIarr_list,HHSSarr,HHSEarr_list,HHStypePopulation,R0min,R0max,corr,infectiousPeriodMean,HHSfact){
  newinfList <- list()
  Sarr2 <- HHSSarr[[hhs]]
  Earr_list2 <- HHSEarr_list[[hhs]]
  pinfls <- pSE(t,hhs,HHSIarr_list,HHStypePopulation,R0min,R0max,corr,infectiousPeriodMean,HHSfact)
  for (ag in 1:4){
    Sarr_ag <- Sarr2[[ag]]
    Earr <- Earr_list2[[ag]]
    pinf <- pinfls[[ag]]
