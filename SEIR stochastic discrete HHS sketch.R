...
Earr_list_init <-  lapply(typePopulation, function(x) lapply(seq_along(latentPeriods),function(x) array(0,dim = c(length(infectiousPeriods),x))))

Iarr_list_init <-  lapply(typePopulation, function(x) lapply(seq_along(infectiousPeriods),function(x) array(0,dim = c(length(latentPeriods),x))))

for (ag in 1:4){
  for (k in seq_along(latentPeriods)){
    larr <- length(Earr_list_init[[ag]][[k]][1,])
    Earr_list_init[[ag]][[k]][,larr] <- E_ag_seedinit[[ag]][,k]
  }
}

for (ag in 1:4){
  for (k in seq_along(infectiousPeriods)){
    Iarr_list_init[[ag]][[k]][,1] <- E_ag_seedinit[[ag]][k,]
  }
}
...
R0min <- runif(1,1.5,2.2)
R0max <- runif(1,R0min,2.2)

corr <- round(runif(1,34-30,34+30)) ## Random peak R0 between ~ Dec 15 and Feb 15

inits <- list(Sarr_init=Sarr_init,Earr_list_init=Earr_list_init,Iarr_list_init=Iarr_list_init)

parameters <- list(inits=inits,corr=corr,R0min=R0min,R0max=R0max,HHStypePopulation=HHStypePopulation,
