...
populationData <- read.csv('pop_state_4.csv')

st_list <- as.vector(unique(populationData$state))
...
pSE <- function(t,hhs,stateIarr_list,statetypePopulation,R0,infectiousPeriodMean,statefact){ ## k is age group, I is matrix of # infectious by latent/infectious time type
  stateI <- sapply(1:4, function(ag) sum(sapply(stateIarr_list[-hhs], function(hhsel) sum(unlist(hhsel[[ag]])))))
  I <- sapply(1:4, function(x) sum(unlist(stateIarr_list[[hhs]][[x]])))
  pop <- sapply(1:4, function(x) sum(statetypePopulation[[hhs]][[x]]))
  statepop <- sapply(1:4, function(ag) sum(sapply(statetypePopulation[-hhs], function(pop) sum(pop[[ag]]))))
  beta <- beta(t,R0,infectiousPeriodMean)
  lambda <- beta * contactMatrix %*% as.numeric(I/pop) + beta*statefact * contactMatrix %*% as.numeric(stateI/statepop) 
  return (as.numeric(1-exp(-lambda)))
}

newInfect <- function(t,hhs,stateIarr_list,stateSarr,stateEarr_list,statetypePopulation,R0,infectiousPeriodMean,statefact){
  newinfList <- list()
  Sarr2 <- stateSarr[[hhs]]
  Earr_list2 <- stateEarr_list[[hhs]]
  pinfls <- pSE(t,hhs,stateIarr_list,statetypePopulation,R0,infectiousPeriodMean,statefact)
  for (ag in 1:4){
    Sarr_ag <- Sarr2[[ag]]
    Earr <- Earr_list2[[ag]]
    pinf <- pinfls[[ag]]
...
dayProgI <- function(hhs,stateEarr_list,stateIarr_list){
  Iarr_list2 <- stateIarr_list[[hhs]]
  for (ag in 1:4){
    for (i in 1:4){
      newinf <- sapply(1:3,function(l) stateEarr_list[[hhs]][[ag]][[l]][i,l])
...
dayProgE <- function(hhs,stateEarr_list){
  Earr_list2 <- stateEarr_list[[hhs]]
  for(ag in 1:4){
    for (l in 1:3){
      if (l > 1){
...
###################################################################################################
###  Construct latent and infecitous period distributions  ########################################
###################################################################################################
...
###################################################################################################
###################################################################################################
statetypePopulation <- list()

for (st in seq_along(st_list)){
state <- st_list[st]
    ls <- sapply(1:4, function(nag) list(sapply(rmultinom(1,populationData[st,nag+1],latentPeriods),function(x) sapply(x, function(y) rmultinom(1,y,infectiousPeriods)))))
  statetypePopulation[[state]] <- ls
}

statepmat <- list()
for (st in seq_along(st_list)){
  state <- st_list[[st]]
  typePopulation <- statetypePopulation[[state]]
  statepmat[[state]] <- sapply(1:4,function(x) list(typePopulation[[state]]/sum(typePopulation[[state]])))
}
