...
population <- colSums(populationData[,2:5])
...
# Has the numbers of individuals by "type" by day since infection
## Function to collect Earr_list into 
pSE <- function(t,I,population,R0min,R0max,corr,infectiousPeriod){ ## k is age group, I is matrix of # infectious by latent/infectious time type
  lambda <- beta(t,R0min,R0max,corr,infectiousPeriod) * contactMatrix %*% as.numeric(I/population)
...
while ( sim < nsim ){
  ...
  serialInterval <- runif(1,2.4,2.8)
  
  latentPeriod <- runif(1,latentPeriod0-.5,latentPeriod0 + .5)
  
  infectiousPeriod <- (serialInterval - latentPeriod)*2
  ...
  Sagarr <-   array(0,dim = c(nsim,4))
  Eagarr <-   array(0,dim = c(nsim,4))
  Iagarr <-   array(0,dim = c(nsim,4))
  
  while (time < durEpidemic){
    pinf <- pSE(time,I,population,R0min,R0max,corr,infectiousPeriod)
